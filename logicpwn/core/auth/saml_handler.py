...
class SAMLConfig(BaseModel):
    ...
    idp_slo_url: Optional[str] = Field(
        default=None, description="IdP Single Logout URL"
    )
    idp_certificate: Optional[str] = Field(
        default=None, description="IdP X.509 certificate for verification"
    )
    ...
    want_response_signed: bool = Field(
        default=False, description="Require signed responses"
    )
...
class SAMLHandler:
    ...
    def create_auth_request(
        self, force_authn: bool = False, is_passive: bool = False
    ) -> tuple[str, str]:
        """
...
<samlp:AuthnRequest
    ...
    <samlp:NameIDPolicy Format="{self.config.name_id_format}" AllowCreate="true"/>
        ...
        if self.config.binding == "urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect":
            ...
            params = {"SAMLRequest": encoded_request, "RelayState": self._relay_state}
    ...
    def _create_post_form_url(self, saml_request: str, relay_state: str) -> str:
        ...
        raise NotImplementedError(
            "SAML POST binding form generation not implemented for production use"
        )
    ...
    def process_saml_response(
        ...
        if relay_state != self._relay_state:
            raise AuthenticationError(
                f"Invalid relay state. Expected: {self._relay_state}, Got: {relay_state}"
            )
        ...
        try:
            ...
            session_index = (
                authn_statement.get("SessionIndex")
            ...
            logger.info(
                f"Successfully processed SAML assertion for subject: {subject_name_id}"
            )
    ...
    def create_logout_request(
        self, name_id: Optional[str] = None, session_index: Optional[str] = None
    ) -> tuple[str, str]:
        """
...
<samlp:LogoutRequest
    ...
    def _verify_saml_signature_warning(
        self, response_root: ET.Element, assertion: ET.Element
    ) -> None:
        """
        ...
        if not response_signature and not assertion_signature:
            if self.config.want_assertions_signed or self.config.want_response_signed:
                logger.error(
                    "SECURITY RISK: No SAML signatures found but signatures required by config"
                )
                raise AuthenticationError(
                    "SAML response/assertion must be signed but no signatures found"
                )
            else:
                logger.warning(
                    "SECURITY WARNING: SAML response and assertion are not signed - vulnerable to tampering"
                )

        if response_signature:
            logger.info(
                "SAML Response signature detected (verification not implemented)"
            )

        if assertion_signature:
            logger.info(
                "SAML Assertion signature detected (verification not implemented)"
            )

        # Basic issuer validation
        if self.config.idp_entity_id:
            assertion_issuer = assertion.find(
                ".//saml:Issuer", {"saml": "urn:oasis:names:tc:SAML:2.0:assertion"}
            )
            if (
                assertion_issuer is not None
                and assertion_issuer.text != self.config.idp_entity_id
            ):
                logger.warning(
                    f"SECURITY WARNING: Assertion issuer {assertion_issuer.text} doesn't match expected {self.config.idp_entity_id}"
                )
...
def create_saml_config_from_metadata(
    sp_entity_id: str, sp_acs_url: str, idp_metadata: IdPMetadata, **kwargs
) -> SAMLConfig:
    """
...
def create_okta_saml_config(
    sp_entity_id: str, sp_acs_url: str, okta_domain: str, app_id: str, **kwargs
) -> SAMLConfig:
...
def create_azure_saml_config(
    sp_entity_id: str, sp_acs_url: str, tenant_id: str, app_id: str, **kwargs
) -> SAMLConfig:
