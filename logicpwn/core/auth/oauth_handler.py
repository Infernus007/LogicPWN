...
class OAuthConfig(BaseModel):
    ...
    redirect_uri: str = Field(
        default="http://localhost:8080/callback", description="Redirect URI"
    )
    scope: list[str] = Field(default_factory=list, description="OAuth scopes")

    # Flow configuration
    grant_type: str = Field(
        default="authorization_code", description="OAuth grant type"
    )
    ...
    userinfo_url: Optional[str] = Field(
        default=None, description="UserInfo endpoint (OIDC)"
    )
    ...
    # Advanced options
    state_generator: Optional[Callable[[], str]] = Field(
        default=None, description="Custom state generator"
    )
    additional_params: dict[str, str] = Field(
        default_factory=dict, description="Additional OAuth parameters"
    )
    token_endpoint_auth_method: str = Field(
        default="client_secret_basic", description="Token endpoint auth method"
    )
...
class OAuthHandler:
    ...
    def get_authorization_url(self) -> tuple[str, str]:
        ...
        if self.config.grant_type not in ["authorization_code", "implicit"]:
            raise ValidationError(
                "Authorization URL only available for authorization_code and implicit flows"
            )
    ...
    def exchange_code_for_token(
        ...
        if self.config.token_endpoint_auth_method == "client_secret_basic":
            ...
            raise ValidationError(
                f"Unsupported token endpoint auth method: {self.config.token_endpoint_auth_method}"
            )

        try:
            response = self.session.post(
                self.config.token_url, data=data, headers=headers, auth=auth, timeout=30
            )
    ...
    def get_user_info(self) -> dict[str, Any]:
        ...
        try:
            response = self.session.get(
                self.config.userinfo_url, headers=headers, timeout=30
            )
            response.raise_for_status()
    ...
    def revoke_token(self, token: Optional[str] = None) -> bool:
        ...
        for revoke_url in revoke_urls:
            try:
                ...
                response = self.session.post(
                    revoke_url, data=data, auth=auth, timeout=10
                )
    ...
    def _validate_state_parameter(self, received_state: str) -> None:
        ...
        if received_state != self._state:
            logger.error(
                f"OAuth state mismatch: expected {self._state}, got {received_state}"
            )
            raise AuthenticationError(
                f"Invalid state parameter. Expected: {self._state}, Got: {received_state}"
            )
...
def create_oauth_config_from_well_known(
    issuer_url: str, client_id: str, client_secret: Optional[str] = None, **kwargs
) -> OAuthConfig:
    """
...
def create_microsoft_oauth_config(
