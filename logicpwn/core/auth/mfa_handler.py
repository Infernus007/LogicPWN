    ...
    def generate_qr_code(self) -> bytes:
        ...
        if not QR_CODE_AVAILABLE:
            raise ValidationError(
                "QR code generation requires 'qrcode' library. Install with: pip install qrcode[pil]"
            )
...
class MFAConfig(BaseModel):
    ...
    totp_period: int = Field(
        default=30, ge=15, le=300, description="TOTP time period in seconds"
    )
    totp_window: int = Field(
        default=1, ge=0, le=5, description="TOTP time window tolerance"
    )

    # SMS settings
    sms_provider: Optional[str] = Field(
        default=None, description="SMS provider (twilio, aws_sns)"
    )
    sms_config: dict[str, Any] = Field(
        default_factory=dict, description="SMS provider configuration"
    )
    sms_template: str = Field(
        default="Your verification code is: {code}", description="SMS message template"
    )

    # Email settings
    email_provider: Optional[str] = Field(
        default=None, description="Email provider (smtp, sendgrid)"
    )
    email_config: dict[str, Any] = Field(
        default_factory=dict, description="Email provider configuration"
    )
    ...
    code_length: int = Field(
        default=6, ge=4, le=10, description="Verification code length"
    )
    code_expiry: int = Field(
        default=300, ge=60, le=3600, description="Code expiry time in seconds"
    )
    max_attempts: int = Field(
        default=3, ge=1, le=10, description="Maximum verification attempts"
    )
    rate_limit_window: int = Field(
        default=3600, description="Rate limiting window in seconds"
    )
    max_codes_per_window: int = Field(
        default=5, description="Maximum codes per rate limit window"
    )

    # Backup codes
    backup_code_count: int = Field(
        default=10, ge=5, le=20, description="Number of backup codes to generate"
    )
    backup_code_length: int = Field(
        default=8, ge=6, le=12, description="Backup code length"
    )
...
class TOTPHandler:
    ...
    def create_totp_secret(
        self, account_name: str, secret: Optional[str] = None
    ) -> TOTPSecret:
    ...
    def _hotp(
        self, secret: str, counter: int, algorithm: str = "SHA1", digits: int = 6
    ) -> str:
        """Generate HOTP code."""
    ...
    def validate_totp(
        self, secret: str, code: str, timestamp: Optional[float] = None
    ) -> bool:
        """
        Validate TOTP code.
...
class SMSHandler:
    ...
    def send_code(self, phone_number: str, code: Optional[str] = None) -> str:
        ...
        if self.config.sms_provider == "twilio":
            ...
            raise ValidationError(
                f"Unsupported SMS provider: {self.config.sms_provider}"
            )
...
class BackupCodeHandler:
    ...
    def generate_backup_codes(self) -> list[str]:
        ...
        for _ in range(self.config.backup_code_count):
            ...
            alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
            code = "".join(
                secrets.choice(alphabet) for _ in range(self.config.backup_code_length)
            )
            codes.append(code)
...
class MFAManager:
    ...
    def _check_rate_limit(self, identifier: str) -> bool:
        ...
        if identifier in self.rate_limit_tracker:
            self.rate_limit_tracker[identifier] = [
                timestamp
                for timestamp in self.rate_limit_tracker[identifier]
        ...
        # Check limit
        return (
            len(self.rate_limit_tracker[identifier]) < self.config.max_codes_per_window
        )
    ...
    def get_challenge_status(self, challenge_id: str) -> Optional[dict[str, Any]]:
        ...
        return {
            ...
            - self.verification_attempts.get(challenge_id, 0),
        }
...
def create_totp_qr_code(
    secret: str, account_name: str, issuer: str = "LogicPwn"
) -> bytes:
    """Create QR code for TOTP setup."""
    totp_secret = TOTPSecret(secret=secret, account_name=account_name, issuer=issuer)
