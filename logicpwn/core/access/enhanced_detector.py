...
from logicpwn.core.logging import log_error, log_info
...
class EnhancedAccessTester:
    ...
    async def run_comprehensive_access_test(
        ...
        try:
            ...
            if (
                self.config.enable_privilege_escalation
                and self.config.current_role_name
            ):
    ...
    async def _run_enhanced_idor_testing(
        ...
        enhanced_config.max_concurrent_requests = min(
            self.config.max_concurrent_tests, enhanced_config.max_concurrent_requests
        )
    ...
    async def _run_tenant_isolation_testing(
        ...
        tenant_config.max_concurrent_tests = min(
            self.config.max_concurrent_tests, tenant_config.max_concurrent_tests
        )
    ...
    async def _run_privilege_escalation_testing(
        ...
        role_config.max_concurrent_tests = min(
            self.config.max_concurrent_tests, role_config.max_concurrent_tests
        )
    ...
    def _calculate_summary_statistics(self, results: EnhancedAccessTestResults) -> None:
        ...
        results.discovered_tenants = list(
            {
                result.target_tenant.tenant_id
                for result in results.tenant_isolation_results
                ...
                "escalations": [
                    {
                        ...
                        "target_role": (
                            result.target_role.role_name if result.target_role else None
                        ),
                        "endpoint": result.endpoint,
...
def create_enhanced_access_config(
    ...
    tenant_config = (
        TenantTestConfig(max_concurrent_tests=max_concurrent)
        if enable_all_features
        else None
    )

    role_config = (
        RoleTestConfig(max_concurrent_tests=max_concurrent)
        if enable_all_features
        else None
    )
