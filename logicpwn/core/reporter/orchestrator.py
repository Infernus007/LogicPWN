...
class ReportGenerator:
    ...
    def generate_report(
        self, format: str = "markdown", template_dir: str = None
    ) -> str:
        """
        ...
        content = exporter.export(
            self.findings, self.metadata, template_dir=template_dir
        )
        return self.redact_sensitive_data(content) if self.redactor else content

    def export_to_file(
        self, filepath: str, format: str, template_dir: str = None
    ) -> None:
        """
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            if hasattr(exporter, "stream_export"):
                exporter.stream_export(
                    self.findings, self.metadata, f, template_dir=template_dir
                )
            else:
    ...
    @cached(ttl=600)
    def collect_findings_from_modules(
        self, auth_results, detector_results, exploit_results
